<template>
    <div className="py-3">
        ...
        <div class="sm:main min-h-[100vh]  md:min-h-[100vh]  sm:border border-gray-300 bg-white  sm:rounded-xl p-4 border-gray-200 xl:min-h-[100vh] 2xl:min-h-[100vh]">    
            <!-- {{ my_designes }} -->
...
>
  My Designes
            ...
            <a-row v-if="view_type=='details' && selected_design">
               ...
               <!-- </div> -->
                    ...
                    <a-col :xs="24" :sm="24" :md="12" :lg="12" style="padding:10px;">
                        <img :src="this.$store.state.root_media_api+selected_design.data.image"  style="width:100%;height:100%;max-height:400px;border-radius:10px;object-fit:cover" alt="">
                    </a-col>
                    <a-col :xs="24" :sm="24" :md="12" :lg="12" style="padding:10px;" v-if="!start_edit">
                        ...
                        <!-- {{ businessUrl(selected_design.data.business_slug) }} -->
                          <br>
                          <br>
                          <p style="font-size:14px;font-weight:500;margin-bottom:4px;color:#666;">
                            Created From
                            </p>
                            ...
                            style="display:flex;align-items:center;gap:8px;padding:6px 10px;border-radius:6px;
                                    text-decoration:none;color:#333;font-weight:600;transition:background 1.2s ease;"
                            >
                            <a-avatar
                                :src="$store.state.root_media_api + selected_design.data.business_picture"
                                style="flex-shrink:0;"
                            />
                            <span style="font-size:15px;font-weight:600;">{{ selected_design.data.business_name }}</span>
                            </router-link>
        ...
        <a-row >
                                            <a-col v-for="product in selected_design.data.products_used" :key="product.id" 
                                                ...
                                                <div class="product">
                                                    ...
                                                    <a-row>
                                                        ...
                                                        <a-col span="6" style="display: flex;justify-content: end;">
                                                            <div v-for="(color, index) in product.product_colors.slice(0, 2)" 
                                                                :key="index"
...
CloseOutlined
    ...
    this.$confirm({
        title: 'Delete Design',
        content: 'Are you sure you want to delete this design? This action cannot be undone.',
        okText: 'Yes, Delete',
        okType: 'danger',
        cancelText: 'Cancel',
        onOk: async () => {
            ...
            try {
                ...
                if (responseData.success || responseData.status === 'success') {
                    ...
                    throw new Error(responseData.message || 'Failed to delete design');
                }
...
async save_Design() {
    ...
    async loadMyDesignes() {
      try {
        ...
        const response = await fetch(`${this.$store.state.root_api}engine/free-user-my-designes/`, {
          method: 'GET',
          headers: {
