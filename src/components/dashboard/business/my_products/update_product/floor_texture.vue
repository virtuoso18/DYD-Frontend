  ...
  <div>
    ...
    >
      <div style="display: flex; align-items: center">
        <a-button
          @click="cancel_edit_back_texture_list"
          type="text"
        ...
        >
          ...
          >
            ...
            <a-button
              ...
              class="!absolute -top-1.5 -right-1.5 !flex !items-center !justify-center !bg-red-500 !text-white !rounded-full !w-5 !h-5 !min-w-5 !p-0"
            >
              <svg
                class="w-3 h-3"
                viewBox="0 0 24 24"
        ...
        <div style="padding-left: 24px; border-left: 1px solid #f3f4f6">
          ...
          <a-row :gutter="16" style="margin-bottom: 20px">
            ...
            <a-col :span="6">
              ...
              >
                <a-select-option
                  v-for="style in textureStyles"
                  :key="style"
                  :value="style"
                  >{{ style }}</a-select-option
                >
              </a-select>
            </a-col>
            <a-col :span="6">
              <label
                style="
                  ...
                  color: #374151;
                "
                >Sale Price per SQM <span style="color: red">*</span></label
              >
              <a-input-number
                v-model:value="textureForm.sale_price_per_sqm"
                :min="0"
                :step="0.01"
                placeholder="20.00"
                suffix="$"
...
export default {
  ...
  computed: {
    primaryImage() {
      ...
      const primaryImg = this.selectedTexture.product_images?.find(
        (img) => img.is_primary,
      );
      if (primaryImg) {
      ...
      if (this.selectedTexture.texture_image) {
        return {
          url:
            ...
            this.selectedTexture.texture_image,
        };
      }
      return null;
    },
      ...
      if (this.selectedTexture) {
        this.textureForm = {
          ...
          installation_cost_per_sqm:
            this.selectedTexture.installation_cost_per_sqm || null,
  ...
  handleRoomTypeChange(value) {
    ...
    isColorAlreadyAdded(colorHex) {
      return (
        this.selectedTexture?.associated_colors?.some(
          (color) => color.color_hex === colorHex,
        ) || false
      );
    },
    ...
    async togglePrimaryImage(imageId) {
      try {
        ...
        if (result.success) {
          // Update local data
          this.selectedTexture.product_images.forEach(
            (img) => (img.is_primary = false),
          );
          const targetImage = this.selectedTexture.product_images.find(
            (img) => img.id === imageId,
          );
      ...
      this.$confirm({
        ...
        onOk: async () => {
          try {
            ...
            const result = await response.json();
            if (result.success) {
              const index = this.selectedTexture.associated_colors.findIndex(
      ...
      this.$confirm({
        ...
        onOk: async () => {
          try {
            ...
            const result = await response.json();
            if (result.success) {
              const index = this.selectedTexture.product_images.findIndex(
                (img) => img.id === imageId,
              );
              if (index !== -1) {
    ...
    validateForm() {
      ...
      if (
        ...
        this.$message.error(
          "Sale Price per SQM is required and must be greater than 0",
        );
        return false;
      }
