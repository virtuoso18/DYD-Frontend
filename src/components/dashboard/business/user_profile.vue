    ...
    <a-row class="dashboard-content">
      <a-col :xs="24" :sm="24" :md="24" :lg="24" class="content-area">
        <div class="sm:content-wrapper">
            ...
            <div class="header-profile">
              <div class="header-avatar">
                <img
                  ...
                  alt=""
                />
                <div v-if="!isProfileImageLoaded" class="avatar-skeleton"></div>
                <img
          ...
          <div class="personal-info-section">
            <div class="section-header">
              ...
              <button class="edit-btn" @click="toggleEdit" :disabled="loading">
                ...
                {{ loading ? t('userProfile.updating') : (isEditing ? t('userProfile.save') : t('userProfile.edit')) }}
              </button>
            </div>
          ...
          <input ref="profilePictureInput" type="file" accept="image/*" style="display: none" @change="handleProfilePictureChange"/>
          <input ref="backgroundPictureInput" type="file" accept="image/*" style="display: none" @change="handleBackgroundPictureChange"/>
...
export default {
  ...
  props: {
    ...
    buisness_info: { type: Object, required: true }
  },
  data() {
    return {
      isEditing: false,
  ...
  methods: {
    ...
    onProfileImageLoad() {
      ...
      setTimeout(() => { this.isProfileImageLoaded = true; }, 1000);
    },

    onWavyHeaderLoad() {
      this.wavyHeaderLoaded = false;
      setTimeout(() => { this.wavyHeaderLoaded = true; }, 1000);
    },
    ...
    async updateProfile() {
      ...
      try {
        ...
        if (this.user_info.first_name) formData.append('first_name', this.user_info.first_name);
        if (this.user_info.last_name) formData.append('last_name', this.user_info.last_name);
        if (this.profile.about) formData.append('about', this.profile.about);
        if (this.selectedProfilePicture) formData.append('profile_picture', this.selectedProfilePicture);
        if (this.selectedBackgroundPicture) formData.append('background_picture', this.selectedBackgroundPicture);
        ...
        if (!response.ok) {
          notification.error({
            message: this.t('userProfile.notifications.updateFailed'),
            description: data.message || this.t('userProfile.notifications.unableToUpdate')
          });
          return;
        }
        ...
        if (!data.error) {
          ...
          notification.success({
            message: this.t('userProfile.notifications.profileUpdated'),
            description: this.t('userProfile.notifications.successfullyUpdated'),
          ...
          notification.error({
            message: this.t('userProfile.notifications.updateError'),
            description: data.message || this.t('userProfile.notifications.somethingWentWrong')
          });
        }
      } catch (error) {
        console.error("Profile update error:", error);
        notification.error({
          message: this.t('userProfile.notifications.serverError'),
          description: this.t('userProfile.notifications.tryAgain')
        });
      } finally {
        this.loading = false;
      }
    ...
    handleProfilePictureChange(event) {
      ...
      if (file) {
        if (file.size > 5 * 1024 * 1024) {
          notification.error({
            message: this.t('userProfile.notifications.fileTooLarge'),
            description: this.t('userProfile.notifications.profilePicLimit')
          });
          return;
        }
        if (!file.type.startsWith('image/')) {
          notification.error({
            message: this.t('userProfile.notifications.invalidFileType'),
            description: this.t('userProfile.notifications.pleaseSelectImage')
          });
          return;
        }
    ...
    handleBackgroundPictureChange(event) {
      ...
      if (file) {
        if (file.size > 10 * 1024 * 1024) {
          notification.error({
            message: this.t('userProfile.notifications.fileTooLarge'),
            description: this.t('userProfile.notifications.backgroundPicLimit')
          });
          return;
        }
        if (!file.type.startsWith('image/')) {
        ...
        reader.onload = (e) => {
          ...
          if (headerElement) headerElement.style.background = `url(${e.target.result}) center/cover no-repeat`;
        };
        reader.readAsDataURL(file);
      }
    }
