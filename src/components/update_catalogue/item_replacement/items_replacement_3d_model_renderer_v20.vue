...
export default {
  ...
  data() {
    return {
    _spinAngleRad: 0,   // ✅ ADD THIS — source of truth for spin, never read from euler
...
_snapRingToChairFast() {
    ...
    if (this.twoFingerPointers.size === 2) {
      ...
      if (!this._twoFingerPending) {
        ...
        this._twoFingerRafId = requestAnimationFrame(() => {
          ...
          // ✅ Go through alignChairToFloor so quaternion is always correct
          this._spinAngleRad = this._twoFingerNewRotY
          this.chairRotation = ((THREE.MathUtils.radToDeg(this._twoFingerNewRotY) % 360) + 360) % 360
          this.alignChairToFloor()   // uses this.chairRotation internally
  ...
  // ✅ Keep both in sync — degrees for UI/watcher, radians as source of truth
  this._spinAngleRad = newRotY
  this.chairRotation = ((THREE.MathUtils.radToDeg(newRotY) % 360) + 360) % 360
...
onPointerUp(event) {
  ...
  // ✅ Convert degrees → radians and store as source of truth
...
offRenderer.setPixelRatio(1)
  ...
  // ✅ Add this inside createCompositeImageBlob, after adding lights
  ...
  // Match the floor normal tilt from live scene
  ...
  offScene.add(shadowPlane)  // ✅ now shadows will render on composite
