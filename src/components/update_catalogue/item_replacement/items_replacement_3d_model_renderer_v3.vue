...
initializeLighting() {
  ...
  // 1. HEMISPHERE LIGHT - Soft ambient illumination
  // Provides overall ambient light without harsh shadows
  const hemisphereLight = new THREE.HemisphereLight(
    0xffffff,  // Sky color (bright white)
    0x8b7355,  // Ground color (warm brown/tan)
    1.2        // Reduced intensity for more natural look
  );
  this.scene.add(hemisphereLight);

  // 2. MAIN DIRECTIONAL LIGHT - Key light (most important)
  // Creates primary shadows and defines form
  const mainLight = new THREE.DirectionalLight(0xffffff, 2.5);
  mainLight.position.set(8, 12, 6);  // Positioned like sun
  mainLight.castShadow = true;
  
  // Optimize shadow map for best quality
  ...
  // Lower intensity, from opposite side to main light
  const fillLight = new THREE.DirectionalLight(0xb8d4ff, 1.0);
  fillLight.position.set(-10, 6, -8);
  this.scene.add(fillLight);

  // 4. BACK/RIM LIGHT - Adds depth and separation
  // Creates rim effect around model edges
  const rimLight = new THREE.DirectionalLight(0xffe8cc, 0.8);
  rimLight.position.set(-5, 8, -12);
  this.scene.add(rimLight);

  // 5. AMBIENT OCCLUSION APPROXIMATION - Contact shadow effect
  // Creates soft shadow where model meets floor
  const contactLight = new THREE.DirectionalLight(0x000000, 0.3);
  contactLight.position.set(0, 0.5, 0);
  this.scene.add(contactLight);
},

// Add this new method to create contact shadows on the floor
createContactShadow() {
  if (!this.model || !this.floorPlane) return;

  // Check if contact shadow already exists
  if (this.contactShadowMesh) {
    this.gridGroup.remove(this.contactShadowMesh);
  }

  // Create a plane for contact shadow
  ...
  this.contactShadowMesh = new THREE.Mesh(shadowGeometry, shadowMaterial);
  
  // Position slightly above floor to avoid z-fighting
  this.contactShadowMesh.position.copy(this.model.position);
  this.contactShadowMesh.position.y += 0.01;
  
  // Rotate to match floor plane
  const up = new THREE.Vector3(0, 1, 0);
  const quaternion = new THREE.Quaternion().setFromUnitVectors(up, this.floorNormal);
  this.contactShadowMesh.quaternion.copy(quaternion);
  
  this.gridGroup.add(this.contactShadowMesh);
},
...
// Update this method to include contact shadow creation
async loadModel() {
  if (!this.glbUrl || this.glbUrl === '') {
      ...
      (gltf) => {
        ...
        this.model.traverse((child) => {
          if (child.isMesh) {
            ...
            // Improve material settings for better lighting
            if (child.material) {
              child.material.shadowSide = THREE.FrontSide;
              // Increase metalness for more reflective surfaces (optional)
...
// Update onMouseMove to refresh contact shadow when dragging
onMouseMove(event) {
  ...
  if (this.isDraggingModel) {
    ...
    if (this.raycaster.ray.intersectPlane(this.dragPlane, intersection)) {
      if (this.isPositionOnFloor(intersection)) {
        ...
        this.updateRotationRingPosition();
        this.createContactShadow();  // Update contact shadow position
...
// Update reset function to include contact shadow
reset_entire_room() {
  if (!this.model) return;
  ...
  this.updateRotationRingPosition();
  this.createContactShadow();  // Update contact shadow on reset
},
