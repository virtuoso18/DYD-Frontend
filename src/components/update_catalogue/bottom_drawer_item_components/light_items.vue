  ...
  <div class="main">
    <a-row>
      <a-col :span="4">
        ...
        <!-- Category Filter -->
        <div style="margin-bottom: 20px; padding-bottom: 15px; border-bottom: 1px solid #f0f0f0;">
          <h4 style="margin: 0 0 8px 0; font-size: 14px; font-weight: 600;">Category</h4>
          ...
          <div v-else style="max-height: 200px; overflow-y: auto; padding-right: 8px;">
            ...
            >
              <span>{{ category.name }} <span style="color: #999; font-size: 11px; margin-left: 4px;">({{ category.product_count }})</span></span>
            </a-checkbox>
            <div v-if="availableCategories.length === 0" style="color: #999; font-size: 12px;">
        ...
        <div style="margin-bottom: 20px; padding-bottom: 15px; border-bottom: 1px solid #f0f0f0;">
          <h4 style="margin: 0 0 8px 0; font-size: 14px; font-weight: 600;">Light Type</h4>
          <div v-if="loadingFilters" style="text-align: center; padding: 10px;">
            <a-spin size="small" />
          </div>
          <div v-else style="max-height: 200px; overflow-y: auto; padding-right: 8px;">
            <a-checkbox 
              ...
              style="display: flex; align-items: center; margin-bottom: 8px;"
            >
              <span>{{ type.furniture_type }} <span style="color: #999; font-size: 11px; margin-left: 4px;">({{ type.product_count }})</span></span>
            </a-checkbox>
            <div v-if="availableFurnitureTypes.length === 0" style="color: #999; font-size: 12px;">
          ...
          <div v-else style="display: flex; gap: 8px; flex-wrap: wrap;">
            <div 
              ...
              :style="{
                ...
                border: filters.selectedColors.includes(color.color) ? '3px solid #1890ff' : '2px solid #ddd',
                cursor: 'pointer',
                transition: 'all 0.2s',
                position: 'relative'
              }"
            ...
            <a-col v-for="product in products" :key="product.id" 
              ...
              <div class="product">
                <div class="product-image-container">
                  ...
                  />
                  <!-- Category Badge -->
                  <div class="category-badge">{{ product.light_type || product.furniture_type }}</div>
                  <!-- AR Badge -->
  ...
  methods: {
    ...
    async loadProducts() {
      try {
        ...
        const selectedCategoryIds = Object.keys(this.filters.selectedCategories)
        ...
        const selectedTypes = Object.keys(this.filters.selectedTypes)
          .filter(type => this.filters.selectedTypes[type])
