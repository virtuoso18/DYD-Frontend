<template>
  <div class="main">
    <div class="products-list">
      <a-row>
      <a-col v-if="!furnitures &&  !lights " style="display:flex;justify-content: center;align-items: center;">
          <a-empty descreption="No Catalog Items Selected"></a-empty>
        </a-col>
        <a-col v-if="!furnitures &&  (catalogItems_Furnitures.length === 0)" style="display:flex;justify-content: center;align-items: center;">
          <a-empty discreption="No Catalog Items Furnitures"></a-empty>
        </a-col>
        <a-col v-if="!lights && (catalogItems_Lights.length === 0)" style="display:flex;justify-content: center;align-items: center;">
          <a-empty discreption="No Catalog Items Lights"></a-empty>
        </a-col>
        <!-- Furniture Products -->
        <a-col v-if="furnitures &&  (catalogItems_Furnitures.length >0)"
        ...
        >
          <div class="product"
            :class="{ 'selected-product': selected_furniture?.id === product.id }"
  @click="selectFurniture(product)" >
            ...
            <a-row>
              ...
              <a-col span="6" style="display: flex; justify-content: end; gap: 2px;">
                ...
                <div 
                  ...
                  style="width: 20px; height: 20px; border-radius: 50%; background: #e0e0e0; display: flex; align-items: center; justify-content: center; font-size: 10px; font-weight: bold; color: #666;"
                >
              ...
              <a-col span="17">
                <a-button block @click="viewProduct(product)" style="margin-top: 8px;">Product Details</a-button>
              </a-col>
              
              <a-col span="1"></a-col>
              <a-col span="6">
                <a-button style="margin-top: 8px; width: 100%;" @click="toggleWishlist(product)">
                  <HeartOutlined :style="{ color: product.isWishlisted ? '#ff4d4f' : 'currentColor' }" />
                </a-button>
              </a-col>
            </a-row>
          </div>
        ...
        <!-- Lights Products -->
        <a-col v-if="lights && (catalogItems_Lights.length >0)" 
          v-for="product in catalogItems_Lights" 
          :key="product.id" 
          class="product-responsive" 
          style="padding: 5px;"
        >
...
export default {
  name: 'switch_furniture',
  components: {
    HeartOutlined
  },
  props: {
    furniture_name: Object,
    lights: Object,
    furnitures: Object,
  },
  data() {
...
switchFurniture() {
  if (this.selected_furniture) {
    this.$emit('switch_furniture', this.selected_furniture)
  } else {
    this.$message.warning('Please select a furniture first')
  }
},
