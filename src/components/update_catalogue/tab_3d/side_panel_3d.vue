  ...
  goToUpgrade() {
    ...
    handleTextureFileSelect(event) {
      ...
      if (file) {
        ...
        reader.onload = (e) => {
          ...
          console.log(
            "Texture file loaded:",
            this.textureImage.substring(0, 50) + "...",
          );
        };
    ...
    async initializeCompletedCount() {
      try {
        const token = localStorage.getItem("token");
        const response = await fetch(
          `${this.$store.state.root_api}engine/queue-status/?room_id=${this.$route.params.id}`,
          {
            method: "GET",
            headers: {
              "Content-Type": "application/json",
    ...
    async addMultiViewToQueue() {
      ...
      try {
        ...
        const token = localStorage.getItem("token");
        const response = await fetch(
          this.$store.state.root_api + "engine/hy-2.0-mv-3D-gen/",
          {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
    ...
    async addToQueue() {
      ...
      try {
        ...
        const token = localStorage.getItem("token");
        const response = await fetch(
          this.$store.state.root_api + "engine/hy-2.0-sv-3D-gen/",
          {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
    ...
    async removeFromQueue(queueId) {
      try {
        const token = localStorage.getItem("token");
        const response = await fetch(
          this.$store.state.root_api + "engine/queue-status/",
          {
            method: "DELETE",
            headers: {
              "Content-Type": "application/json",
    ...
    //         if (view.image) {
    //           const baseImg= await this.urlToBase64(view.image);
    //           imagesToUpload.push({ type: `view-${index}`, data: baseImg });
    //         }
    //       });
