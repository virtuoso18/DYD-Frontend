  ...
  <div class="loading-screen" :style="{ backgroundImage: `url(${backgroundImageUrl})` }">
    ...
    <div class="loading-text">
      ...
      <div
        ...
        :class="[
          `light-${light.type}`,
          { 'selected': light.id === selectedLightId, 'dragging': isDraggingLight && draggedLight?.id === light.id }
        ]"
      ...
      >
        <div class="light-body">
          ...
          <div class="light-indicator" v-if="light.id === selectedLightId"></div>
        </div>
      </div>
      ...
      <div class="light-editor" @click.stop>
        <div class="editor-header">
          <h3>
            ...
            Edit {{ editingLight.type }} Light #{{ lights.findIndex(l => l.id === editingLight.id) + 1 }}
          </h3>
          ...
          <div class="editor-section">
            ...
            <div class="editor-grid">
              ...
              <div class="editor-item">
                ...
                <div class="input-group">
                  <input type="range" v-model.number="editingLight.brightness" min="0.1" max="1" step="0.1" class="slider">
                  <span class="value">{{ Math.round(editingLight.brightness * 100) }}%</span>
                </div>
              </div>
              <div class="editor-item">
                <label>Color Temperature</label>
                <div class="input-group">
                  <input type="range" v-model.number="editingLight.temperature" min="2700" max="6500" step="100" class="slider">
...
function drawLightToExport(exportCtx, light, scaleX, scaleY) {
  ...
  if (x < 0 || y < 0 || x > exportCtx.canvas.width || y > exportCtx.canvas.height) {
    return;
  }
  ...
  glowGradient.addColorStop(0, `${lightColor}${Math.round(glowAlpha * 255).toString(16).padStart(2, '0')}`);
  glowGradient.addColorStop(0.3, `${lightColor}${Math.round(glowAlpha * 0.5 * 255).toString(16).padStart(2, '0')}`);
  ...
  if (light.type === 'circular') {
    ...
    exportCtx.strokeRect(-finalWidth / 2, -finalHeight / 2, finalWidth, finalHeight);
  }
...
function calculateImageDimensions() {
  ...
  const imgAspectRatio = backgroundImage.value.width / backgroundImage.value.height;
...
function handleGlobalMouseMove(e) {
  if (isDraggingLight.value && draggedLight.value) {
    ...
    const projected = projectPointOnTrack(mouseX, mouseY, draggedLight.value.track);
...
function handleTrackClick(pos) {
  if (!isDrawingTrack.value) {
    ...
    if (trackLength > 10) { // Minimum track length
...
function findNearestTrack(x, y, threshold = 30) {
  ...
  tracks.value.forEach(track => {
    const distance = distanceToLine(x, y, track.x1, track.y1, track.x2, track.y2);
...
function deleteLight() {
  ...
  const lightIndex = lights.value.findIndex(l => l.id === editingLight.value.id);
  if (lightIndex > -1) {
...
function getDepthAtPosition(x, y) {
  ...
  const maskX = Math.floor(((x - renderOffsetX.value) / renderWidth.value) * depthMaskImage.value.width);
  const maskY = Math.floor(((y - renderOffsetY.value) / renderHeight.value) * depthMaskImage.value.height);
  
  const clampedX = Math.max(0, Math.min(maskX, depthMaskImage.value.width - 1));
  const clampedY = Math.max(0, Math.min(maskY, depthMaskImage.value.height - 1));
...
function redrawCanvas() {
  ...
  if (isDrawingTrack.value && currentTrack.value) {
    ...
    if (hasEndPoint) {
      ...
      ctx.value.fillRect(midX - 25 / zoom.value, midY - 8 / zoom.value, 50 / zoom.value, 16 / zoom.value);
      
      // Length text
      ctx.value.fillStyle = 'white';
      ctx.value.font = `${10 / zoom.value}px Arial`;
      ctx.value.textAlign = 'center';
      ctx.value.fillText(`${Math.round(length)}px`, midX, midY + 3 / zoom.value);
      ctx.value.textAlign = 'left';
    }
    
    // Draw start point with pulsing animation
    const startPointSize = 8 / zoom.value;
    const pulseSize = startPointSize + (Math.sin(Date.now() * 0.005) * 2 / zoom.value);
    
    // Outer glow
    ctx.value.fillStyle = 'rgba(255, 149, 0, 0.3)';
    ctx.value.beginPath();
    ctx.value.arc(currentTrack.value.x1, currentTrack.value.y1, pulseSize + 4 / zoom.value, 0, 2 * Math.PI);
    ctx.value.fill();
    
    // Main start point
    ctx.value.fillStyle = '#363636';
    ctx.value.beginPath();
    ctx.value.arc(currentTrack.value.x1, currentTrack.value.y1, startPointSize, 0, 2 * Math.PI);
    ctx.value.fill();
    
    // Inner highlight
    ctx.value.fillStyle = 'rgba(255, 255, 255, 0.8)';
    ctx.value.beginPath();
    ctx.value.arc(currentTrack.value.x1, currentTrack.value.y1, startPointSize * 0.4, 0, 2 * Math.PI);
    ctx.value.fill();
    ...
    if (hasEndPoint) {
      ...
      ctx.value.beginPath();
      ctx.value.arc(currentTrack.value.x2, currentTrack.value.y2, 6 / zoom.value, 0, 2 * Math.PI);
      ctx.value.fill();
      ctx.value.stroke();
    }
  }
...
.light-body {
  ...
  box-shadow: 
    ...
    0 0 16px rgba(255, 255, 240, 0.6),
    0 0 32px rgba(255, 255, 240, 0.4);
}
