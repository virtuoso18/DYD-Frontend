<template>
  <div>
    ...
    <a-alert type="info" :description="t('productDetails.notLoggedIn')" v-if="anonyomusUser === null" />
    ...
    <a-row :gutter="[24, 24]">
      ...
      <a-col :xs="24" :md="12">
        <!-- 3D Model Card -->
        <div style="position: relative; padding: 10px; height: 400px;">
  
        <!-- 3D Model View -->
        <div v-if="activeView === '3d'" style="height: 100%;">
        ...
        <div
          ...
          style="height: 100%; border-radius: 10px; display: flex; align-items: center; justify-content: center; background: #f5f5f5;"
        >
          <img
          ...
          <div style="display: flex; gap: 8px; flex-wrap: wrap">
            <div
              ...
              :style="{
                ...
                borderRadius: '10px', display: 'flex', flexDirection: 'center',
            ...
            <div
              ...
              :style="{
                ...
                border: activeView === 'image' && activeImageIndex === index
                  ? '3px solid #1890ff' : '1px solid #d9d9d9',
                cursor: 'pointer', transition: 'all 0.3s ease', overflow: 'hidden',
              }"
            >
              <img :src="$store.state.root_media_api + img.image"
      ...
      <a-col :xs="24" :md="12">
        <a-card>
          ...
          <div style="margin-bottom: 20px">
            <h4 style="font-weight: normal">{{ t('productDetails.dimensions') }}</h4>
            <span v-if="selectedProduct.dimensions.width" style="font-weight: bold">
              {{ selectedProduct.dimensions.width }} {{ t('productDetails.meter') }}
              {{ t('productDetails.width') }} &nbsp;
            </span>
            <span v-if="selectedProduct.dimensions.height" style="font-weight: bold">
              {{ selectedProduct.dimensions.height }} {{ t('productDetails.meter') }}
              {{ t('productDetails.height') }} &nbsp;
            </span>
            <span v-if="selectedProduct.dimensions.length" style="font-weight: bold">
              {{ selectedProduct.dimensions.length }} {{ t('productDetails.meter') }}
          ...
          <a-row :gutter="16" style="margin-bottom: 24px">
            <a-col span="24">
              <a-alert
                v-if="showColorAlert"
                type="warning"
                :message="t('productDetails.colorAlertMsg')"
                closable
              ...
              <div style="display: flex; gap: 6px; flex-wrap: wrap">
                <img
                  ...
                  style="width: 40px; height: 40px; border-radius: 4px;
                    border: 1px solid #d9d9d9; object-fit: cover;"
                />
              </div>
            </a-col>
          </a-row>
...
export default {
  ...
  methods: {
    openWhatsApp() {
      const businessPhone = this.selectedProduct?.product_business_owner_phone;
      const productName = this.selectedProduct.name;
      const productLink = `${this.windowLocation}/store/${this.$route.params.buisness_name}/product/${this.selectedProduct.id}`;

      const message =
        `Hi, I'm interested in this product I saw on DYD.%0A` +
        `%F0%9F%AA%91 Product: ${encodeURIComponent(productName)}%0A` +
