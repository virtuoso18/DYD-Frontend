         ...
         <div class="flex-1 overflow-y-auto px-6 py-4" ref="messagesContainer">
          ...
          <div v-else class="flex flex-col gap-4">
            ...
            >
              ...
              <div v-if="message.user != currentUser.id" class="mr-3 flex-shrink-0">
                <a-avatar 
                  :size="32"
                  style="border:1px solid rgba(0,0,0,0.3)"
                  :src="this.$store.state.root_media_api + message.userImage"
              ...
              <div class="max-w-[70%]">
                ...
                <div v-if="message.user != currentUser.id" class="text-xs text-gray-500 mb-1 px-4">
                  {{ message.userName }}
                </div>
                ...
                <div class="text-xs text-gray-500 mt-1" :class="message.user == currentUser.id ? 'text-right' : ''">
                  {{ formatTime(message.timestamp) }}
                </div>
              </div>

              <!-- Avatar for sent messages -->
              <div v-if="message.user == currentUser.id" class="ml-3 flex-shrink-0 gap:">
                <a-avatar 
                  :size="35"
                  style="border:1px solid rgba(0,0,0,0.3)"
...
export default {
    ...
    watch: {
        ...
        '$route.query.access_id'(newVal, oldVal) {
            if (newVal && newVal !== oldVal) {
                this.accessId = newVal;
                this.loadAccessChatRooms();
            }
        }
    },
    ...
    methods: {
        async openRoomFromQuery() {
            ...
            console.log("Opening room from query:", roomId);

            if (roomId) {
                if (!this.userRooms || this.userRooms.length === 0) {
                    await this.loadAccessChatRooms();
                }
        ...
        async selectRoom_by_query(roomId) {
            console.log("Selecting room by query:", roomId);
            if (!roomId) return;
            ...
            if (room.userProfilsAvatar.user_1.id !== this.currentUser.id) {
                ...
                new_user = room.userProfilsAvatar.user_2;
            }
        ...
        async initializeAccessMessages() {
            this.accessId = this.$route.query.access_id;
            if (!this.accessId) {
                console.error('No access_id provided');
                return;
            }
            await this.loadAccessChatRooms();
            this.setupWebSocket();
        ...
        async loadAccessChatRooms() {
            try {
                const token = localStorage.getItem('token');
                if (!this.accessId) {
                    console.error('access_id is required');
                    return;
                }
                
                const response = await fetch(`${this.API_BASE}/chats/active?access-id=${this.accessId}`, {
                    method: 'GET',
                    headers: { 'Authorization': `Token ${token}` }
                });
                this.userRooms = await response.json();
            } catch (error) {
                console.error('Error loading access chat rooms:', error);
            }
        },
        ...
        async loadMessages(roomId) {
            if (!roomId) return;
            try {
                const response = await fetch(`${this.API_BASE}/chats/${roomId}/messages?access-id=${this.accessId}`);
        ...
        async markMessages_seen(roomId) {
            try {
                const token = localStorage.getItem('token');
                const response = await fetch(`${this.API_BASE}/mark-recieved-all-messages-seen/?access-id=${this.accessId}`, {
                    method: 'POST',
        ...
        setupWebSocket() {
            ...
            const wsUrl = `${this.$store.state.websockets_address}/ws/users/${this.currentUser.id}/chat/?token=${token}&access-id=${this.accessId}`;
            console.log('Connecting WebSocket with access_id');
        ...
        get_proper_user_avatar(getProper_user_avatar) {
            if (getProper_user_avatar.user_1.id !== this.currentUser.id) {
                ...
                return getProper_user_avatar.user_2;
            }
        },
