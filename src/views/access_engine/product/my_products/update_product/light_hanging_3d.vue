  ...
  <div>
    ...
    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 24px; padding: 0 10px;">
      <div style="display: flex; align-items: center;">
        ...
        <h2 style="margin: 0; font-size: 20px; font-weight: 600; color: #1f2937;">Edit Product</h2>
      </div>
    ...
    <a-row style="padding: 0 24px;">
      ...
      <a-col :xs="24" :sm="24" :md="12" :lg="12">
        ...
        <p style="color: #6b7280; font-size: 13px; margin-bottom: 24px;">Upload 3D model (.glb, .gltf up to 50MB)</p>
        <div style="padding-right: 24px;">
          <div style="margin-bottom: 32px;">
            ...
            <div v-else @click="upload3DModel" @dragover.prevent="dragOver" @dragleave.prevent="dragLeave" @drop.prevent="drop3DModel"
      ...
      <a-col :span="24">
        <div style="padding-left: 24px; border-left: 1px solid #f3f4f6;">
          <h3 style="margin-bottom: 24px; font-weight: 600; font-size: 16px; color: #1f2937;">Product Details</h3>
...
export default {
  ...
  methods: {
      ...
      if (this.selectedProduct) {
        this.productForm = {
          ...
          pricing: {
            ...
            sale_price: this.selectedProduct.pricing?.sale_price?.toString() || ''
          },
          dimensions: {
          ...
          colors: { primary_color: this.selectedProduct.colors?.primary_color || '' }
        };
    ...
    cancel_edit_back_product_list() {
      if (this.hasUnsavedChanges) {
        this.$confirm({
          title: 'Unsaved Changes', content: 'You have unsaved changes. Are you sure you want to leave without saving?',
          okText: 'Leave', okType: 'danger', cancelText: 'Stay',
    ...
    validate3DModelFile(file) {
      ...
      if (!validExtensions.includes(extension)) {
        this.$message.error('Invalid file type. Please upload .glb or .gltf files only.');
        return false;
      }
      if (file.size > 50 * 1024 * 1024) {
        this.$message.error('File size too large. Please upload files smaller than 50MB.');
        return false;
      }
      return true;
    },
    ...
    // Drag and Drop
    dragOver(event) { event.preventDefault(); this.isDragOver = true; },
    dragLeave(event) { event.preventDefault(); this.isDragOver = false; },
    drop3DModel(event) {
    ...
    remove3DModel() {
      this.$confirm({
        title: 'Remove 3D Model', content: 'Are you sure you want to remove the 3D model?',
    ...
    selectTexture(textureId) { this.selectedTexture = textureId; },

    // Primary Image Toggle
    async togglePrimaryImage(imageId) {
      try {
        const token = localStorage.getItem('token');
        const response = await fetch(`${this.$store.state.root_api}access-engine/api/business-products/products/${this.selectedProduct.id}/images/${imageId}/set-primary/`, {
          method: 'PATCH',
    ...
    async addColor(colorHex) {
      ...
      try {
        const token = localStorage.getItem('token');
        const response = await fetch(`${this.$store.state.root_api}access-engine/api/business-products/products/${this.selectedProduct.id}/colors/`, {
    ...
    async addTextureToProduct(textureFile) {
      try {
        ...
        const response = await fetch(`${this.$store.state.root_api}access-engine/api/business-products/products/${this.selectedProduct.id}/textures/`, {
          method: 'POST', headers: { 'Authorization': `Token ${token}` }, body: formData
        });

        const result = await response.json();
        if (result.success) {
          this.selectedProduct.textures.push({ id: result.data.id, texture: result.data.texture });
    ...
    async deleteImage(imageId) {
      this.$confirm({
        title: 'Delete Image', content: 'Are you sure you want to delete this image?',
        okText: 'Delete', okType: 'danger', cancelText: 'Cancel',
        onOk: async () => {
          try {
            const token = localStorage.getItem('token');
            const response = await fetch(`${this.$store.state.root_api}access-engine/api/business-products/products/${this.selectedProduct.id}/images/${imageId}/`, {
    ...
    async deleteColor(colorId) {
      this.$confirm({
        ...
        onOk: async () => {
          try {
            const token = localStorage.getItem('token');
            const response = await fetch(`${this.$store.state.root_api}access-engine/api/business-products/products/${this.selectedProduct.id}/colors/${colorId}/`, {
              method: 'DELETE', headers: { 'Authorization': `Token ${token}` }
            });
    ...
    async deleteTexture(textureId) {
      this.$confirm({
        ...
        onOk: async () => {
          try {
            const token = localStorage.getItem('token');
            const response = await fetch(`${this.$store.state.root_api}access-engine/api/business-products/products/${this.selectedProduct.id}/textures/${textureId}/`, {
              method: 'DELETE', headers: { 'Authorization': `Token ${token}` }
            });
    ...
    async saveProduct() {
      ...
      try {
        ...
        if (this.imagePreviewsState.length > 0) {
          ...
          const imageResponse = await fetch(`${this.$store.state.root_api}access-engine/api/business-products/products/${this.selectedProduct.id}/images/`, {
            method: 'POST', headers: { 'Authorization': `Token ${token}` }, body: formData
          });
        ...
        if (this.productForm.colors.primary_color) {
          productData.append('primary_color', this.productForm.colors.primary_color);
        }
        ...
        const response = await fetch(`${this.$store.state.root_api}access-engine/api/business-products/products/${this.selectedProduct.id}/`, {
