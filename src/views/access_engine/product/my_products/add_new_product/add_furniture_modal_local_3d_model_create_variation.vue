    ...
    <template #title>
      <div style="display: flex; align-items: center; justify-content: space-between; padding: 0;">
        <div style="display: flex; align-items: center;">
          ...
          <span class="sm:text-[18px] text-[14px] !font-semibold text-[#1f2937]">Add New Product</span>
        </div>
      ...
      <a-col :xs="24" :sm="24" :md="10" :lg="10">
        <div style="position: relative;">
          ...
          <div style="display: flex; flex-wrap: wrap; gap: 8px;">
            <img v-if="modelDetails?.image_0" :src="this.$store.state.root_media_api + modelDetails.image_0"
              :style="{ width: '60px', height: '60px', objectFit: 'contain', borderRadius: '8px', border: '2px solid #e5e7eb' }" />
            <img v-if="modelDetails?.image_1" :src="this.$store.state.root_media_api + modelDetails.image_1"
              :style="{ width: '60px', height: '60px', objectFit: 'contain', borderRadius: '8px', border: '2px solid #e5e7eb' }" />
            <img v-if="modelDetails?.image_2" :src="this.$store.state.root_media_api + modelDetails.image_2"
              :style="{ width: '60px', height: '60px', objectFit: 'contain', borderRadius: '8px', border: '2px solid #e5e7eb' }" />
            <img v-if="modelDetails?.image_3" :src="this.$store.state.root_media_api + modelDetails.image_3"
              :style="{ width: '60px', height: '60px', objectFit: 'contain', borderRadius: '8px', border: '2px solid #e5e7eb' }" />
          </div>
        </div>
      </a-col>
        ...
        <div style="padding-left: 12px;">
          ...
          <div style="margin-bottom: 20px;">
            ...
            <div style="display: flex; gap: 8px; align-items: center;">
              <div
                style="width: 60px; height: 60px; background: #f3f4f6; border-radius: 8px; display: flex; align-items: center; justify-content: center; cursor: pointer; margin-top: 14px;"
                @click="uploadImages"
              >
              ...
              <div v-if="selectedImages.length > 0" style="margin-top: 16px;">
                <div style="display: flex; flex-wrap: wrap; gap: 8px;">
                  ...
                  >
                    ...
                    <a-button type="text" size="small" @click.stop="removeImage(index)"
          ...
          <a-row :gutter="12" style="margin-bottom: 16px;">
            ...
            <!-- Category (disabled until room type is selected) -->
            <a-col :span="6">
              <div style="margin-bottom: 16px;">
                <label style="display: block; margin-bottom: 6px; font-size: 13px; color: #374151;">
                ...
                <div style="display: grid; grid-template-columns: repeat(6, 32px); gap: 8px; padding: 8px;">
                  <div
                    ...
                    :style="{
                      ...
                      backgroundColor: color, cursor: 'pointer', border: '1px solid #e5e7eb',
            ...
            <div v-if="selectedColors.length > 0" style="margin-top: 12px;">
              <div style="display: flex; flex-wrap: wrap; gap: 8px;">
                <div v-for="(color, index) in selectedColors" :key="index" style="position: relative;">
                  ...
                  <a-button type="text" size="small" @click="removeColor(index)"
                    style="position: absolute; top: -8px; right: -8px; background: #ef4444; color: white; border-radius: 50%; width: 20px; height: 20px; padding: 0; min-width: 20px;">
                    <template #icon>
                      <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3">
                        <line x1="18" y1="6" x2="6" y2="18"></line>
...
export default {
  ...
  props: {
    ...
    prepopulatedData: { type: Object, default: () => null },
  },
  ...
  methods: {
    ...
    addPresetTexture(texture) {
      ...
      if (index === -1) {
        this.selectedTextures.push({ id: texture.id, url: texture.url, name: texture.name });
      } else {
    ...
    async loadInitialCategories() {
      try {
        ...
        const result = await response.json();
        console.log('API Response (categories):', result);
        if (result.success) {
          this.allCategories = result.data || [];
          this.categoryOptions = this.allCategories.map(cat => ({
            label: cat.name, value: cat.name, data: cat
          }));
      ...
      this.categorySearchTimeout = setTimeout(async () => {
        try {
          ...
          const response = await fetch(
            `${this.$store.state.root_api}product/api/categories/search/?q=${encodeURIComponent(searchValue)}${roomTypeParam}`,
    ...
    initializeFormWithDefaults(defaults) {
      console.log('initializeFormWithDefaults', defaults);
      this.productForm = {
        name: defaults.name || '',
    ...
    validate3dModelFile(file) {
      ...
      if (!validTypes.includes(fileExtension)) { console.error(`Invalid type for ${file.name}.`); return false; }
      if (file.size > maxSize) { console.error(`${file.name} is too large.`); return false; }
      return true;
    },
    ...
    async get3dRenderedModelDetails(generated3dModelId) {
      ...
      try {
        ...
        if (responseData.success) {
          ...
          console.log('✅ 3D Model Details:', this.modelDetails);
    ...
    removeImage(index) {
      ...
      if (removed.isPrimary && this.selectedImages.length > 0) this.selectedImages[0].isPrimary = true;
    },
    ...
    removeColor(index) {
      const removed = this.selectedColors[index];
      this.selectedColors.splice(index, 1);
      if (removed.isPrimary && this.selectedColors.length > 0) this.selectedColors[0].isPrimary = true;
    },
    ...
    handleTextureUpload(event) {
      ...
      files.forEach(file => {
        if (this.validateImageFile(file)) {
          const reader = new FileReader();
          reader.onload = (e) => {
            // id: null marks this as a custom upload (gets the 📤 badge)
    ...
    validateImageFile(file) {
      ...
      if (!validTypes.includes(file.type)) { console.error(`Invalid type for ${file.name}.`); return false; }
      if (file.size > maxSize) { console.error(`${file.name} is too large.`); return false; }
      return true;
    },
    ...
    async handleSave() {
      ...
      try {
        ...
        // 3D model — uploaded file takes priority over existing rendered ID
        ...
        // Room type
        if (this.selectedRoomTypeName) formData.append('room_type_name', this.selectedRoomTypeName);
        ...
        // Textures: separate custom uploads from library IDs
        const textureIds = [];
        ...
        console.log('📤 Sending product data:', {
          ...
          room_type_name: this.selectedRoomTypeName,
        ...
        const response = await fetch(
          `${this.$store.state.root_api}access-engine/api/business-products/add-product-furniture/?access-id=` + this.$route.query.access_id,
          {
            method: 'POST',
