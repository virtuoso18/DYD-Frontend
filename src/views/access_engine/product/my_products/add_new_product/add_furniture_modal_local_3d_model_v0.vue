...
Pellentesque habitant morbi tristique senectus et netus et malesuada fames ac turpis egestas."
            ...
            <!-- Category Type, Category, Type, Price Row -->
<a-row :gutter="12" style="margin-bottom: 16px;">
  <!-- Category Type Dropdown -->
  <a-col :span="6">
    <label style="display: block; margin-bottom: 6px; font-size: 13px; color: #374151;">Category Type <span style="color: red;">*</span></label>
    <a-select 
      v-model:value="productForm.category_type" 
  ...
  <!-- Category Dropdown (dependent on Category Type) -->
  <a-col :span="6">
    <label style="display: block; margin-bottom: 6px; font-size: 13px; color: #374151;">Category <span style="color: red;">*</span></label>
    <a-select 
      v-model:value="productForm.category_name" 
      ...
      style="width: 100%;"
      :style="{ background: '#f3f4f6' }"
      :disabled="!productForm.category_type"
    >
      <a-select-option 
...
const category_types = computed(() => {
  return Object.keys(categories_available.value).sort();
});

const available_categories = computed(() => {
  if (!productForm.value.category_type) {
    return [];
  }
  return categories_available.value[productForm.value.category_type] || [];
});

// Handler for category type change
const handleCategoryTypeChange = () => {
  productForm.value.category_name = ''; // Reset category when type changes
};
...
const fetch_Categories_available = async () => {
  try {
    ...
    console.log('📡 Fetching Product categories Available...');
    ...
    if (responseData.success) {
      // The API returns: { "Living Room": [...], "Ceiling Lights": [...] }
      const groupedData = responseData.data || {};
      console.log('✅ Categories Available (Grouped):', groupedData);
      categories_available.value = groupedData;
      return groupedData;
    } else {
