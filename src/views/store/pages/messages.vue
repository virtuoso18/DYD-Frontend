    ...
    <div class="p-1 bg-white rounded-2xl border border-gray-200">
      <div class="flex h-screen overflow-auto bg-gray-50">
            ...
            <div
              ...
              @click="
                selectRoom(
                  ...
                  get_proper_user_avatar(room.userProfilsAvatar).avatar +
                    '?t=' +
                    new Date().getTime(),
                )
              "
              ...
              <div v-if="!room.show_live_support_icon" class="mr-4 flex-shrink-0">
                ...
                >
                  <a-avatar
                    ...
                    :src="
                      $store.state.root_media_api +
                      room.userProfilsAvatar.user_1.avatar
                    "
                  />
                  <a-avatar
                    style="border: 1px solid rgba(0, 0, 0, 0.2)"
                    :src="
                      $store.state.root_media_api +
                      room.userProfilsAvatar.user_2.avatar
                    "
                  />
                  <a-avatar
                    style="border: 1px solid rgba(0, 0, 0, 0.2)"
                    :src="
                      $store.state.root_media_api +
                      room.userProfilsAvatar.user_3.avatar
                    "
                  />
                </a-avatar-group>
              </div>
        ...
        >
          ...
          <div
            v-if="currentRoomId"
            class="px-6 py-4 border-b border-gray-200 bg-white sticky top-0 z-10"
          >
            ...
            <div v-else class="flex flex-col gap-4">
              ...
              >
                <div class="max-w-[70%]">
                  ...
                  >
                    <a-avatar
                      :src="$store.state.root_media_api + message.userImage"
                    ></a-avatar>
                    {{ message.message }}
                  </div>
  ...
  methods: {
    ...
    async loadUserRooms() {
      try {
        ...
        const response = await fetch(
          `${this.API_BASE}/users/${this.currentUser.id}/chats`,
          {
            method: "GET",
    ...
    leaveCurrentRoom() {
      if (this.websocket?.readyState === WebSocket.OPEN && this.currentRoomId) {
        this.websocket.send(
          JSON.stringify({ action: "leave_room", roomId: this.currentRoomId }),
        );
      }
    },
    ...
    joinRoom(roomId) {
      if (this.websocket?.readyState === WebSocket.OPEN) {
        this.websocket.send(
          JSON.stringify({ action: "join_room", roomId: roomId }),
        );
      }
    },
    ...
    async loadMessages(roomId) {
      ...
      try {
        const response = await fetch(
          `${this.API_BASE}/chats/${roomId}/messages`,
        );
        const data = await response.json();
        this.messages = Array.isArray(data.results || data)
    ...
    async markMessages_seen(roomId) {
      try {
        const token = localStorage.getItem("token");
        const response = await fetch(
          `${this.API_BASE}/mark-recieved-all-messages-seen/`,
          {
            method: "POST",
            headers: {
      ...
      this.websocket.onopen = () => {
        ...
        setTimeout(() => {
          this.userRooms.forEach((room) => {
            if (room.id) {
              this.websocket.send(
                JSON.stringify({ action: "join_room", roomId: room.id }),
              );
            }
          });
        }, 500);
    ...
    handleMessagesSeen(data) {
      ...
      const room = this.userRooms.find(
        (r) => String(r.id) === String(data.roomId),
      );
      if (room) {
    ...
    updateRoomLastMessage(messageData) {
      const room = this.userRooms.find(
        (r) => String(r.id) === String(messageData.roomId),
      );
      if (room) {
    ...
    updateOnlineUsers(userList) {
      this.onlineUsers = this.allUsers.filter((user) =>
