<template>
  <a-modal
    :open="get_businesss_location_map_modal"
    @close="get_businesss_location_map_modal = false"
  >
    <MapLocationSelector
    ...
    />
      ...
      <div class="!text-gray-700" style="padding: 10px">
        <div style="background: #f3f3f3; border-radius: 10px; padding: 10px">
          ...
          <div v-else>
            <div
              v-for="(service, index) in editData.services"
              :key="index"
...
.business-bg-wrapper {
  position: relative;
  overflow: hidden;
  height: 200px; /* fixed height for both skeleton and content */
}

/* SKELETON FULL COVER */
.business-bg-skeleton {
  position: absolute;
  ...
  animation: business-bg-shimmer 1.6s infinite linear;
  z-index: 1;
}
...
/* SHIMMER KEYFRAMES (STANDARD) */
@keyframes business-bg-shimmer {
  to {
