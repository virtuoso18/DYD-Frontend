  ...
  <Teleport to="body">
    <transition name="modal">
      ...
      >
            ...
            <div class="flex justify-between items-center py-2 text-gray-600 w-full">
              ...
              <div class="flex flex-row gap-6">
                ...
                <div class="flex items-center gap-2 cursor-pointer" @click="toggleLike">
                  <svg v-if="localIsLiked" width="16" height="16" viewBox="0 0 16 16" fill="#EF4444" class="transition-colors duration-200" xmlns="http://www.w3.org/2000/svg">
                    <path d="M6.94307 13.3105C5.06222 11.904 1.33594 8.68856 1.33594 5.79499C1.33594 3.88245 2.73944 2.33203 4.66927 2.33203C5.66927 2.33203 6.66927 2.66536 8.0026 3.9987C9.33594 2.66536 10.3359 2.33203 11.3359 2.33203C13.2657 2.33203 14.6693 3.88245 14.6693 5.79499C14.6693 8.68856 10.943 11.904 9.06214 13.3105C8.4292 13.7838 7.576 13.7838 6.94307 13.3105Z" stroke="#EF4444" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
                  </svg>
                  <svg v-else width="16" height="16" viewBox="0 0 16 16" fill="none" stroke="#4D4D4D" stroke-width="1.5" class="transition-colors duration-200 hover:stroke-red-500" xmlns="http://www.w3.org/2000/svg">
                    <path d="M6.94307 13.3105C5.06222 11.904 1.33594 8.68856 1.33594 5.79499C1.33594 3.88245 2.73944 2.33203 4.66927 2.33203C5.66927 2.33203 6.66927 2.66536 8.0026 3.9987C9.33594 2.66536 10.3359 2.33203 11.3359 2.33203C13.2657 2.33203 14.6693 3.88245 14.6693 5.79499C14.6693 8.68856 10.943 11.904 9.06214 13.3105C8.4292 13.7838 7.576 13.7838 6.94307 13.3105Z" stroke-linecap="round" stroke-linejoin="round"/>
                  </svg>
            ...
            <div class="flex-1 min-w-0 overflow-y-auto px-4 py-3">
              ...
              >
                <div class="flex gap-3">
                  <img :src="comment.avatar" alt="" class="w-10 h-10 rounded-full flex-shrink-0" />
        ...
        <!-- Mobile Modal -->
        <div
          class="md:hidden bg-white rounded-t-3xl w-full flex flex-col absolute left-0 right-0"
        ...
        >
          ...
          <div class="flex-1 min-w-0 overflow-hidden">
            ...
            <!-- Description + Tags + Products (scrollable) -->
            <div class="px-4 py-2 overflow-y-auto no-scrollbar max-h-[40vh]">
              <p class="text-gray-600 text-sm mb-3" v-html="post.description"></p>

              <!-- Tags -->
              <div class="flex gap-2 flex-wrap mb-4">
                <span v-for="tag in post.tags" :key="tag" class="px-3 py-1 bg-gray-100 text-gray-700 text-xs rounded-md">
                  {{ tag }}
                </span>
              </div>
          ...
          <div class="border-t border-gray-200 px-4 py-3 bg-white shrink-0" @click.stop>
            <div class="flex items-center gap-3">
...
export default {
  ...
  components: { RoomProductsSection },
  ...
  methods: {
    ...
    onHandleTouchMove(e) {
      ...
      const diff = this.dragStartY - e.touches[0].clientY;
      if (diff > 60 && !this.isMobileExpanded)  this.isMobileExpanded = true;
      if (diff < -60 && this.isMobileExpanded)  this.isMobileExpanded = false;
    },
    ...
    handleViewportResize() {
      ...
      this.mobileModalHeight = Math.max(vh * 0.85, '70vh');
    },
    ...
    async loadComments(page = 1) {
      ...
      try {
        ...
        if (data.success) {
          ...
          if (page === 1) this.apiComments = comments;
          else            this.apiComments.push(...comments);
      ...
      try {
        const response = await fetch(
          `${this.$store.state.root_api}community/api/comments/`,
          {
            ...
            body: JSON.stringify({ post_id: this.post.id, content: this.newComment.trim() }),
          }
        );
        const data = await response.json();
        if (data.success) {
